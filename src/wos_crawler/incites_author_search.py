...
post_url = 'https://incites.clarivate.com/incites-app/explore/0/person/data/table/page'
...
post_data = {"take": 100000, "skip": 0, "sortBy": "timesCited", "sortOrder": " desc",
             "filters": {"personIdTypeGroup": {"is": "name"}, "personIdType": {"is": "fullName"},
                         "personId": {"is": ["Li Xiaoli", "Li, Xiaoli", "Li, Xiao-Li", "Li Xiao-li", "Xiaoli, Li"]},
                         "authorposition": {"is": ["First"]}, "clbrprsnIdTypeGroup": {"is": "name"},
                         "clbrprsnIdType": {"is": "fullName"}, "schema": {"is": "Web of Science"},
                         "publisherType": {"is": "All"}, "fundingAgencyType": {"is": "All"},
                         "articletype": {"is": ["Article", "Review"]}, "period": {"is": [2010, 2019]}}, "pinned": [],
             "indicators": ["key", "seqNumber", "prsnName", "rank", "affiliation", "researcherId", "orcId",
                            "wosDocuments", "percentCited", "timesCited", "norm", "hasAffiliation"]}
...
def get_doc_data(input_data, author_position, period):
    ...
    if period == '近5年Q1':
        post_data['filters']['jrnname'] = {"is": q1_list}
    ...
    while count < len(input_data):
        ...
        if result_df_list:
            ...
            write2sql([['incites_author_data', all_data_df]])

    if no_result:
        no_result_df = pd.DataFrame(data=no_result, columns=['人才编号', '姓名', '作者类型', '文献数量'])
        no_result_df.to_excel('C:/Users/Administrator/Desktop/no_result0922.xlsx', index=False)
...
if __name__ == '__main__':
    ...
    period_list = ['近10年', '近5年', '近5年Q1']
    author_position_list = ['First', 'Corresponding', 'First & Corresponding', 'All']
