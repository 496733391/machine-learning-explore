...
dbutil = DBUtil(host, port, database, username, password)
sql = 'select person_id, affiliation, timesCited, doc_num, author_position, period from incites_author_data'
...
result['doc_num'] = result['doc_num'].astype('int')
result = result.groupby(by=['人才编号', '姓名', 'author_position', 'period'], as_index=False).sum()
df = df.loc[~df['人才编号'].isin(list(result['人才编号']))]
df = pd.merge(df, data, left_on=['人才编号'], right_on=['person_id'], how='inner')
df.to_excel('C:/Users/Administrator/Desktop/no_result0922.xlsx', index=False)
result.to_excel('C:/Users/Administrator/Desktop/result0922.xlsx', index=False)
