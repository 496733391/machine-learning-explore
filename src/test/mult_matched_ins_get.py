...
if __name__ == '__main__':
    input_df = pd.read_excel('C:/Users/Administrator/Desktop/test0611.xlsx')

    result_df = main_prog(input_df)
    result_df = result_df.loc[:, ['person_id', 'scopus_person_id', 'affiliationName']]

    data_all = pd.read_excel('C:/Users/Administrator/Desktop/test_data/test_data2.xlsx')
    data_all.drop_duplicates(subset=['学者代码'], inplace=True, ignore_index=True)

    data_all['百度搜索链接'] = 'https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&tn=baidu&wd=' + data_all['头衔当选单位'] + data_all['姓名']
    data_all['谷歌搜索链接'] = 'https://www.google.com/search?&q=' + data_all['头衔当选单位'] + data_all['姓名']

    result_df = pd.merge(result_df, data_all, how='left', left_on='person_id', right_on='学者代码')

    result_df.set_index(['person_id', '头衔当选单位', '姓名', '百度搜索链接', '谷歌搜索链接', 'scopus_person_id', 'affiliationName'], inplace=True)
    result_df.to_excel('C:/Users/Administrator/Desktop/0611mult.xlsx', encoding='utf8')
