...
def match(cookies, person_id, author_name, author_ins, authorID_list):
    '''
    ...
    for author_id in authorID_list:
        ...
        if len(set(author_ins).difference(set(institute_list))) == 1:
            logger.info('匹配的机构中只差一个的：软科id：%s, 姓名：%s, scopus_id：%s' % (person_id, author_name, author_id))
            not_matched_one.append(author_id)

        # 机构完全匹配的
        if len(set(author_ins).difference(set(institute_list))) == 0:
            logger.info('匹配的记录：软科id：%s, 姓名：%s, scopus_id：%s' % (person_id, author_name, author_id))
